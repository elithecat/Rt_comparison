#!/usr/bin/env Rscript
# ─────────────────────────────────────────────────────────────────────────────
# Stochastic SIR renewal-process simulation with EpiEstim Wallinga-Teunis R_t
#
# - R_t = 1.1 for t in [0, 50), R_t = 1.2 for t in [50, 100]
# - Generation interval: Gamma(mean=5, sd=2.5)
# - 200 simulations averaged
# - Uses EpiEstim::wallinga_teunis for R_t estimation
# ─────────────────────────────────────────────────────────────────────────────
...
# ── EpiEstim Wallinga-Teunis estimator wrapper ───────────────────────────────
estimate_wt_epiestim <- function(inc) {
  # EpiEstim expects incidence starting from day 0
  # wallinga_teunis returns estimates for each time point

  # Create the serial interval distribution for EpiEstim
  # EpiEstim uses si_distr starting from 0 (probability at lag 0, 1, 2, ...)
  # We prepend 0 for lag 0 (no same-day infection)
  si_distr <- c(0, gi_pmf)

  # Define time windows - each day gets its own window
  n_days <- length(inc)
  t_start <- 2:n_days  # Start from day 2 (need at least one prior day)
  t_end <- t_start     # Single-day windows

  # Run EpiEstim's wallinga_teunis with non-parametric SI
  wt_result <- wallinga_teunis(
    ...
    config = list(
      ...
      t_end = t_end,
      si_distr = si_distr,
      n_sim = 100  # Number of simulations for uncertainty
    )
  )

  # Extract mean R estimates
  # The result has R estimates aligned to time points
  rt_est <- rep(NA_real_, length(inc))

  # wallinga_teunis returns a data frame with t_start, t_end, Mean(R), etc.
...
# ── Run simulations ──────────────────────────────────────────────────────────
cat("Running 200 simulations with EpiEstim Wallinga-Teunis...\n")
...
while (successful < N_SIMS) {
  ...
  # Use EpiEstim's wallinga_teunis
  rt_est <- tryCatch(
    estimate_wt_epiestim(inc),
    error = function(e) {
      warning(paste("EpiEstim error in sim", successful, ":", e$message))
...
# ── Plot ─────────────────────────────────────────────────────────────────────
png("/workspace/sir_wt_epiestim_plot.png", width = 10, height = 7.5, units = "in", res = 200)
...
# ── Panel B: EpiEstim Wallinga-Teunis R_t ────────────────────────────────────
plot(NULL, xlim = c(0, T_MAX), ylim = c(0.5, 2.0),
     xlab = "Time (days)", ylab = expression(Reproduction ~ number ~ R[t]),
     main = "", axes = FALSE)
title(main = expression(bold("B") ~ "  EpiEstim Wallinga–Teunis reproduction number"),
      adj = 0, cex.main = 1.25, line = 1)
axis(1); axis(2, las = 1)
...
legend("topright",
       legend = c("Mean EpiEstim WT estimate", expression(True ~ R[t]),
...
dev.off()
cat("Plot saved to /workspace/sir_wt_epiestim_plot.png\n")
